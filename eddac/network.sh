...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating AA1 Identities"
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-aa1.yaml --output=organizations
    { set +x; } 2>/dev/null
    res=$?
    verifyResult $res "Failed to generate certificates for AA1..."


    infoln "Creating AA2 Identities"
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-aa2.yaml --output=organizations
    { set +x; } 2>/dev/null
    res=$?
    verifyResult $res "Failed to generate certificates for AA2..."

    infoln "Creating AA3 Identities"
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-aa3.yaml --output=organizations
    { set +x; } 2>/dev/null
    res=$?
    verifyResult $res "Failed to generate certificates for AA3..."

    infoln "Creating DT Identities"
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-dt.yaml --output=organizations
    { set +x; } 2>/dev/null
    res=$?
    verifyResult $res "Failed to generate certificates for DT..."
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.aa1.example.com docker_peer0.aa2.example.com docker_peer0.aa3.example.com docker_peer0.dt.example.com  docker_peer1.dt.example.com docker_peer2.dt.example.com docker_peer3.dt.example.com docker_peer4.dt.example.com
